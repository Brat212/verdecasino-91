...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-a4ef1f94";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-a4ef1f94">'+'<span class="tile-face-a4ef1f94 tile-front-a4ef1f94"></span>'+'<span class="tile-face-a4ef1f94 tile-back-a4ef1f94"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-a4ef1f94")){return;}
var back=tile.querySelector(".tile-back-a4ef1f94");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-a4ef1f94");if(isMine){tile.classList.add("boom-a4ef1f94");}
if(dim){tile.classList.add("dimmed-a4ef1f94");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-a4ef1f94")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-a4ef1f94");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-a4ef1f94")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-a4ef1f94");void panelEl.offsetWidth;panelEl.classList.add("jolt-a4ef1f94");}
endRound();}
